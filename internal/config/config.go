...
// Tag 取自环境变量 FLEET_IMAGE_TAG，未设置时为 v1.3.0；镜像名为 {repo}:{tag}-runner。
...
type RunnersConfig struct {
	...
	ContainerImage   string `yaml:"container_image"`   // Runner 容器镜像，未填时由 DefaultRunnerContainerImage() 决定（FLEET_IMAGE_TAG 或 v1.3.0）

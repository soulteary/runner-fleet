...
> Self-hosted runners should serve **private repositories only** — a pull request to a public repository can run code on this machine. See [SECURITY.md](SECURITY.md).

Open http://localhost:8080. The default image tag is the stable release (e.g. v1.9.0). For more options (docker run, DinD, container mode, using `main` or other tags) see the [User Guide](docs/guide.md). Probes, Prometheus metrics and logging: [Operations](docs/guide.md#5-operations) — `GET /health` (liveness), `GET /ready` (readiness), `GET /metrics`, `GET /version`.

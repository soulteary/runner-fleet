...
- `docs/ci-recipes-migration.md` records whether this repository's shell belongs in [soulteary/ci-recipes](https://github.com/soulteary/ci-recipes), which replaces per-repository CI shell with one tested Go binary. Of six shell surfaces here, two clear that project's own boundary — it excludes runtime entrypoints, examples and inline workflow shell, which rules out `install-runner.sh`, the standalone deploy example, the `detect-go-module-root` composite action and the 17 multi-line `run:` blocks across five workflows. The two that remain are the two `scripts/check-*.sh` files that `CI (Consistency)` runs. The recommendation is to migrate them, but after ci-recipes publishes tagged binaries: today the move would add `setup-go`, an unversioned commit pin and an 18-second build of all four repositories' recipes to a job whose entire cost is a checkout, to buy correctness properties the in-shell fixes below have already secured. Both recipes now exist there as `runner-fleet check-version-consistency` and `runner-fleet check-docs-structure`, with the per-repository facts — the baseline path and regex, the ignore marker, the scanned Git pathspecs, the documentation root, languages and files — read from a `scripts/ci-recipes.conf` in this repository rather than compiled into the binary, so that adding a seventh UI language stays a commit here instead of a release there. Output parity was verified against this tree on both paths: a passing run is byte-identical, and a failing run differs only in the remediation hint saying to rerun the check rather than the script. This repository has not switched yet, and the note says why — ci-recipes publishes no tags, so the only thing to pin is a bare commit SHA — and carries the workflow diff to land once it does. It also argues that the file most worth rewriting in Go is the one ci-recipes will not take: `install-runner.sh`, whose `curl | sed | awk` sits on the download's trust boundary and belongs in `internal/runner` beside the registration job that calls it.

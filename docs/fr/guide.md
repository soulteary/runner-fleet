...
- **L'interface et ses messages sont traduits ; les logs ne le sont pas.** L'interface et ce que le serveur répond — messages d'API et notifications — suivent votre langue, choisie via `?lang=`, le cookie de langue ou `Accept-Language` ; un script qui n'envoie pas `Accept-Language` obtient l'anglais. **Les lignes de log restent en chinois**, autotest de démarrage compris ; elles sont préfixées `[preflight …]` afin d'être trouvables sans lire le chinois. Quelques messages issus de couches plus profondes (le résultat du désenregistrement GitHub, par exemple) laissent encore passer du chinois. C'est une limite connue, pas une traduction inachevée.

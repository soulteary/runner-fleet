...
En production, utilisez une version précise (ex. v1.6.0). En développement, vous pouvez utiliser le tag `main`.

```bash
docker pull ghcr.io/soulteary/runner-fleet:v1.6.0
```
...
docker run -d --name runner-manager \
  ...
  ghcr.io/soulteary/runner-fleet:v1.6.0
```
...
runners:
  ...
  container_mode: true
  container_image: ghcr.io/soulteary/runner-fleet:v1.6.0-runner
...
docker build -f Dockerfile.runner -t ghcr.io/soulteary/runner-fleet:v1.6.0-runner .
```
...
| `runners.container_image` | Image runner en mode conteneur (tag -runner) | `ghcr.io/soulteary/runner-fleet:v1.6.0-runner` |

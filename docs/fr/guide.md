...
- **L'interface et ses messages suivent votre langue ; les logs sont fixés en anglais.** L'interface et ce que le serveur répond — messages d'API et notifications — sont choisis via `?lang=`, le cookie de langue ou `Accept-Language` ; un script qui n'envoie pas `Accept-Language` obtient l'anglais. Les logs sont en anglais par décision, non par omission : une ligne de log n'a aucune requête à suivre, son lecteur est celui qui exploite le déploiement, et grep, les requêtes Loki et les règles d'alerte se cassent dès que le même événement apparaît dans une langue différente. L'autotest de démarrage va aussi dans le log, préfixé `[preflight …]`.
...
En production, utilisez une version précise (ex. v1.8.0). En développement, vous pouvez utiliser le tag `main`.
...
Image runner : même nom que le Manager avec le tag `-runner` (production : version ex. v1.8.0-runner ; dev : main-runner), ou build local : `docker build -f Dockerfile.runner -t ghcr.io/soulteary/runner-fleet:v1.8.0-runner .`. Le Manager doit utiliser le Docker hôte (montage de `docker.sock`), pas DinD via `DOCKER_HOST` ; dans Compose, utilisez `group_add` pour le GID docker hôte ou `user: "0:0"`. Avec `job_docker_backend: host-socket`, le Manager passe `--group-add <GID docker hôte>` au conteneur runner (détecté automatiquement depuis `docker.sock`, remplaçable via `runners.docker_gid` / `DOCKER_GID`) ; l'image embarque aussi un groupe `docker` (arg de build `DOCKER_GID`, 999 par défaut). Les noms de runner sont normalisés en noms de conteneurs ; les doublons après mapping entreront en conflit.

**`runner-net` est une frontière de confiance** : avec `job_docker_backend: dind`, le service DinD accepte des connexions non authentifiées sur le port 2375 depuis n'importe quoi sur `runner-net`, et c'est un conteneur privileged partagé par tous les runners — un job peut voir et entrer dans les conteneurs que d'autres jobs y ont lancés. Réservez `runner-net` au Manager, à DinD et aux conteneurs runner ; si des runners servent des dépôts qui ne se font pas mutuellement confiance, donnez-leur `job_docker_backend: none`, ou séparez les déploiements.
...
| `runners.container_image` | Image runner en mode conteneur (tag -runner) | `ghcr.io/soulteary/runner-fleet:v1.8.0-runner` |
...
| `RUNNER_IMAGE`, `CONTAINER_IMAGE` | `runners.container_image` | Non définies : dérivé de `MANAGER_IMAGE` (`:v1.8.0` → `:v1.8.0-runner`), sinon de `FLEET_IMAGE_TAG` |
...
| `FLEET_IMAGE_TAG` | Tag de l'image runner par défaut quand rien d'autre ne le décide | `v1.8.0` |
...
**Résultat d'enregistrement** : Écrit dans `.registration_result.json` dans le répertoire du runner. **Vérification de visibilité GitHub** (optionnel) : Placez le PAT dans `config/tokens/<nom du runner>` (mode 0600 ; org nécessite `admin:org`, repo nécessite `repo`) ; vérifié ~toutes les 5 minutes, résultat dans `.github_status.json`. Un PAT laissé dans le répertoire du runner par une version antérieure y est déplacé automatiquement puis supprimé du répertoire du runner — ce répertoire est monté dans le conteneur du runner, où les jobs pourraient le lire. Le même contrôle enregistre aussi si GitHub indique que le runner **exécute un job** ; la liste affiche alors un badge « Occupé » et la boîte de dialogue une ligne dédiée. Il partage la cadence d’environ 5 minutes et peut donc avoir jusqu’à 5 minutes de retard ; sans PAT la valeur reste « inconnu » plutôt que signalée comme inactive. Dans la liste, « Enregistré » et « GitHub ✓ » renvoient tous deux vers la page des runners de la cible. Le même fichier note aussi si le dépôt cible est **public** : un dépôt public porte alors un badge « Dépôt public » de couleur d'avertissement dans la liste et une ligne « Visibilité du dépôt » dans la boîte de dialogue, car un dépôt public permet à quiconque peut ouvrir une pull request d'exécuter du code ici. Cette question n'est posée qu'une fois par jour et par dépôt — la visibilité change rarement et l'API anonyme n'autorise que 60 requêtes par heure — et elle fonctionne sans PAT, en en utilisant un lorsque le runner en a un. Les cibles d'organisation ne sont pas couvertes : il faudrait y déterminer si le groupe de runners autorise les dépôts publics, ce qui exige `admin:org`.
...
**Supprimer un runner** : il est arrêté, désenregistré de GitHub si son répertoire contient un PAT, et **son répertoire d'installation est supprimé** — uniquement si ce répertoire se trouve sous `runners.base_path`, afin qu'un chemin mal configuré n'emporte pas un répertoire système. `_work` et les caches qu'il contient disparaissent aussi : rien n'est récupérable ensuite.
...
**Environnement transmis aux runners** : le Manager et l'Agent retirent `BASIC_AUTH_PASSWORD`, `BASIC_AUTH_USER` et `AGENT_TOKEN` de l'environnement de chaque processus qu'ils démarrent — `run.sh` et les jobs qu'il exécute, `config.sh`, `install-runner.sh`. Tout le reste est transmis inchangé, y compris `DOCKER_HOST` et les réglages de proxy. Cela garde les identifiants du Manager hors des journaux des jobs ; ce n'est pas une frontière d'isolation : en mode par défaut, un job s'exécute sous le même utilisateur que le Manager (voir [SECURITY.md](../../SECURITY.md)).

**Fichiers sensibles** : config/config.yaml, .env et `config/tokens/` sont dans `.gitignore`. Le PAT optionnel de chaque runner se trouve dans `config/tokens/<nom du runner>`, que le Manager crée en 0700 avec le fichier en 0600 — ne le laissez pas dans le répertoire du runner, que le mode conteneur monte dans le conteneur du runner où les jobs pourraient le lire.
...
**Le mode par défaut est un seul domaine de confiance** : les permissions du répertoire ci-dessus protègent les identifiants d'un runner des autres utilisateurs de la machine, pas des autres runners. En mode par défaut, tous les runners et le Manager s'exécutent sous un seul utilisateur dans un seul conteneur : un job sur n'importe quel runner peut donc lire les identifiants et le PAT de tous les autres et modifier la configuration du Manager ; avec le fichier compose fourni, il atteint aussi le socket Docker de l'hôte. Donnez à chaque runner son propre conteneur (`runners.container_mode: true`) lorsque les runners servent des dépôts ou des propriétaires différents, ou lorsque l'un d'eux détient un PAT. Si vous restez en mode par défaut et que les jobs n'ont pas besoin de Docker, retirez le volume `docker.sock` et `group_add` de `docker-compose.yml`, et construisez avec `--build-arg ALLOW_SUDO=false`. Voir [SECURITY.md](../../SECURITY.md).
...
Sauvegardez le répertoire `config/` (`config.yaml` ainsi que `tokens/`) et chaque répertoire `runners/<nom>/`, en excluant `_work/`.
...
| `config/tokens/<nom>` (hors du répertoire du runner) | Vous, optionnellement | La vérification de visibilité s'arrête, et supprimer le runner ne peut plus le désenregistrer de GitHub |

...
- **Oberfläche und Meldungen sind übersetzt, die Logs nicht.** Die UI und das, was der Server zurückgibt — API-Meldungen und Toasts — folgen Ihrer Sprache, ermittelt aus `?lang=`, dem Sprach-Cookie oder `Accept-Language`; ein Skript ohne `Accept-Language` erhält Englisch. **Logzeilen sind weiterhin chinesisch**, auch der Startup-Selbsttest; sie tragen das Präfix `[preflight …]`, damit man sie ohne Chinesischkenntnisse findet. Einige Meldungen aus tieferen Schichten (etwa das Ergebnis der GitHub-Abmeldung) führen noch chinesischen Text mit. Eine bekannte Grenze, keine unfertige Übersetzung.

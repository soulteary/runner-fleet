...
**Registrierungsergebnis**: Wird in `.registration_result.json` im Runner-Verzeichnis geschrieben. **GitHub-Sichtbarkeitsprüfung** (optional): Das PAT nach `config/tokens/<Runner-Name>` legen (Modus 0600; Org braucht `admin:org`, Repo braucht `repo`); wird ca. alle 5 Minuten geprüft, Ergebnis in `.github_status.json`. Ein PAT, das eine frühere Version im Runner-Verzeichnis hinterlassen hat, wird automatisch dorthin verschoben und aus dem Runner-Verzeichnis entfernt — dieses Verzeichnis wird in den Container des Runners gemountet, wo Jobs es lesen könnten. Dieselbe Prüfung erfasst auch, ob GitHub den Runner als **Job-ausführend** meldet; in der Liste erscheint dann ein „Beschäftigt“-Badge, im Konfigurationsdialog eine eigene Zeile. Sie teilt sich den ~5-Minuten-Takt und kann daher bis zu 5 Minuten nachhinken; ohne PAT bleibt der Wert „unbekannt“ statt als inaktiv gemeldet zu werden. „Registriert“ und „GitHub ✓“ in der Liste verlinken beide auf die Runners-Einstellungsseite des Ziels. Dieselbe Datei hält außerdem fest, ob das Ziel-Repository **öffentlich** ist: Ein öffentliches trägt in der Liste ein warnfarbenes „Öffentliches Repo“-Badge und im Dialog eine Zeile „Repository-Sichtbarkeit“ — denn ein öffentliches Repository lässt jeden, der einen Pull Request öffnen kann, hier Code ausführen. Diese Frage wird pro Repository höchstens einmal täglich gestellt — die Sichtbarkeit ändert sich selten und die anonyme API erlaubt nur 60 Anfragen pro Stunde — und sie funktioniert ohne PAT, nutzt aber eines, wenn der Runner eines hat. Organisationsziele sind nicht abgedeckt: Dort wäre zu beantworten, ob die Runner-Gruppe öffentliche Repositories zulässt, und das erfordert `admin:org`.
...
**An Runner übergebene Umgebung**: Manager und Agent entfernen `BASIC_AUTH_PASSWORD`, `BASIC_AUTH_USER` und `AGENT_TOKEN` aus der Umgebung jedes Prozesses, den sie starten — `run.sh` und die davon ausgeführten Jobs, `config.sh`, `install-runner.sh`. Alles andere wird unverändert durchgereicht, einschließlich `DOCKER_HOST` und Proxy-Einstellungen. Das hält die Zugangsdaten des Managers aus den Job-Logs heraus; es ist keine Isolationsgrenze — im Standardmodus läuft ein Job unter demselben Benutzer wie der Manager (siehe [SECURITY.md](../../SECURITY.md)).
...
**Der Standardmodus ist eine einzige Vertrauenszone**: Die Verzeichnisrechte oben schützen die Zugangsdaten eines Runners vor anderen Benutzern auf dem Host, nicht vor anderen Runnern. Im Standardmodus laufen alle Runner und der Manager als ein Benutzer in einem Container: Ein Job auf einem beliebigen Runner kann daher die Zugangsdaten und das PAT jedes anderen Runners lesen und die Konfiguration des Managers ändern; mit der mitgelieferten Compose-Datei erreicht er zusätzlich den Docker-Socket des Hosts. Geben Sie jedem Runner seinen eigenen Container (`runners.container_mode: true`), wenn die Runner unterschiedliche Repositories oder Owner bedienen oder wenn einer von ihnen ein PAT hält. Wenn Sie beim Standardmodus bleiben und die Jobs kein Docker brauchen, entfernen Sie das `docker.sock`-Volume und `group_add` aus `docker-compose.yml` und bauen Sie mit `--build-arg ALLOW_SUDO=false`. Siehe [SECURITY.md](../../SECURITY.md).

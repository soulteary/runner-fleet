...
Production: use a specific version (e.g. v1.9.0). For development you can use the `main` tag.
...
No need to edit config/config.yaml. Copy `cp .env.example .env` and set e.g. `CONTAINER_MODE=true`, `VOLUME_HOST_PATH=<host absolute path to runners>` (e.g. `realpath runners`), `JOB_DOCKER_BACKEND=host-socket`, `CONTAINER_NETWORK=runner-net`. If you do not create `config/config.yaml`, the program will generate it on first start from these env vars. If `RUNNER_IMAGE` is unset, the runner image is derived from `MANAGER_IMAGE` (e.g. `v1.9.0` → `v1.9.0-runner`). Mounted `config` and `runners` still need `chown 1001:1001`. See `.env.example` for all override variables.
...
Runner image: same name as Manager with `-runner` tag (production: use a version tag e.g. v1.9.0-runner; dev: main-runner), or build locally: `docker build -f Dockerfile.runner -t ghcr.io/soulteary/runner-fleet:v1.9.0-runner .`. Manager must use host Docker (mount `docker.sock`), not DinD via `DOCKER_HOST`; in Compose use `group_add` for host docker GID or `user: "0:0"`. For `job_docker_backend: host-socket`, the Manager passes `--group-add <host docker GID>` to the runner container (auto-detected from `docker.sock`, override with `runners.docker_gid` / `DOCKER_GID`); the image also ships a `docker` group (build arg `DOCKER_GID`, default 999). Runner names are normalized to container names; duplicates after mapping will conflict.

**`runner-net` is a trust boundary**: with `job_docker_backend: dind`, the DinD service accepts unauthenticated connections on port 2375 from anything on `runner-net`, and it is a privileged container shared by every runner — a job can see and enter containers other jobs started there. Keep `runner-net` for the Manager, DinD and runner containers only; if runners serve mutually untrusted repositories, give them `job_docker_backend: none`, or separate deployments.
...
- **A runner you deleted is still listed on GitHub, and adding it back under the same name fails**: deletion now also deregisters it from GitHub, but only when `config/tokens/<runner name>` holds the optional PAT (organization needs `admin:org`, repository needs `repo`). Without one there is no credential to do it with, and the delete response says so and points at Settings → Actions → Runners. Runners deleted by older versions were never deregistered; remove those by hand.
...
| `runners.container_image` | Runner image in container mode (tag with -runner) | `ghcr.io/soulteary/runner-fleet:v1.9.0-runner` |
...
| `RUNNER_IMAGE`, `CONTAINER_IMAGE` | `runners.container_image` | Unset: derived from `MANAGER_IMAGE` (`:v1.9.0` → `:v1.9.0-runner`), else from `FLEET_IMAGE_TAG` |
...
| `FLEET_IMAGE_TAG` | Tag for the default runner image when nothing else decides it | `v1.9.0` |
...
**Private repositories only**: a runner registered to a public repository — or to an organization runner group with **Allow public repositories** turned on — runs workflows from anyone who can open a pull request. Runners here are persistent: the work directory, the tool caches under it and `$HOME` survive from one job to the next, so a single untrusted job can affect every later one. Register runners only for private repositories you trust, and leave untrusted code to GitHub-hosted runners. See [SECURITY.md](../SECURITY.md).
...
**Registration result**: Written to `.registration_result.json` in that runner dir. **GitHub visibility check** (optional): Put the PAT in `config/tokens/<runner name>` (mode 0600; org needs `admin:org`, repo needs `repo`); checked ~every 5 minutes, result in `.github_status.json`. A PAT left in the runner directory from an earlier version is moved there automatically and removed from the runner directory — that directory is mounted into the runner's container, where jobs could read it. The same check also records whether GitHub says the runner is **running a job**, shown as a “Busy” badge in the list and as its own row in the config dialog. It shares that ~5-minute cadence, so it can lag by up to 5 minutes; without the PAT it stays “unknown” rather than being reported as idle. The list’s “Registered” and “GitHub ✓” both link to that target’s Runners settings page on GitHub. The same file also records whether the target repository is **public**: a public one carries a warn-coloured “Public repo” badge in the list and a “Repository visibility” row in the dialog, because a public repository lets anyone who can open a pull request run code here. That one is asked at most once a day per repository — visibility rarely changes and the anonymous API allows only 60 requests an hour — and it works without a PAT, using one when the runner has one. Organization targets are not covered: what would have to be answered there is whether the runner group allows public repositories, which needs `admin:org`.
...
**Environment passed to runners**: the Manager and the Agent remove `BASIC_AUTH_PASSWORD`, `BASIC_AUTH_USER` and `AGENT_TOKEN` from the environment of every process they start — `run.sh` and the jobs it runs, `config.sh`, `install-runner.sh`. Everything else is passed through unchanged, including `DOCKER_HOST` and proxy settings. This keeps the Manager's credentials out of job logs; it is not an isolation boundary — in the default mode a job runs as the same user as the Manager (see [SECURITY.md](../SECURITY.md)).

**Sensitive files**: config/config.yaml, .env and `config/tokens/` are in `.gitignore`. Each runner's optional PAT lives in `config/tokens/<runner name>`, which the Manager creates mode 0700 with the file mode 0600 — keep it out of the runner directory, which container mode mounts into the runner's container where jobs could read it.
...
Back up the `config/` directory (`config.yaml` plus `tokens/`) and each `runners/<name>/` directory, excluding `_work/`.
...
| `config/tokens/<name>` (outside the runner directory) | You, optionally | The visibility check stops, and deleting the runner can no longer deregister it from GitHub |

...
- **화면과 그 메시지는 번역되었지만 로그는 아닙니다.** UI와 서버가 돌려주는 것——API 메시지와 토스트——은 언어를 따릅니다. `?lang=`, 언어 cookie, `Accept-Language` 순으로 정해지며, `Accept-Language`를 보내지 않는 스크립트는 영어를 받습니다. **로그 줄은 여전히 중국어**이고 시작 자가 점검도 마찬가지입니다. 줄 앞에 `[preflight …]`가 붙어 중국어를 몰라도 찾을 수 있습니다. 더 깊은 층에서 생기는 일부 메시지(GitHub 등록 해제 결과 등)에는 중국어가 남아 있습니다. 알려진 한계이며 번역이 덜 된 것이 아닙니다.

...
운영 환경에서는 특정 버전(예: v1.9.0)을 사용하세요. 개발 시에는 `main` 태그를 쓸 수 있습니다.
...
config/config.yaml 수정 없이 사용. `cp .env.example .env` 후 예: `CONTAINER_MODE=true`, `VOLUME_HOST_PATH=<runners 호스트 절대 경로>`(예: `realpath runners`), `JOB_DOCKER_BACKEND=host-socket`, `CONTAINER_NETWORK=runner-net` 설정. `config/config.yaml`을 만들지 않아도 위 변수를 `.env`에 설정해 두면 첫 실행 시 자동 생성됩니다. `RUNNER_IMAGE`를 설정하지 않으면 Runner 이미지는 `MANAGER_IMAGE`에서 자동 유도(예: v1.9.0 → v1.9.0-runner). 마운트한 `config`와 `runners`는 여전히 `chown 1001:1001` 필요. 자세한 내용은 `.env.example`의 오버라이드 변수 참조.
...
Runner 이미지: Manager와 동일한 이름에 `-runner` 태그(운영: 버전 예 v1.9.0-runner, 개발: main-runner), 또는 로컬 빌드: `docker build -f Dockerfile.runner -t ghcr.io/soulteary/runner-fleet:v1.9.0-runner .`. Manager는 호스트 Docker(`docker.sock` 마운트)를 사용해야 하며, `DOCKER_HOST`로 DinD를 사용하면 안 됩니다. Compose에서는 호스트 docker GID용 `group_add` 또는 `user: "0:0"`을 사용하세요. `job_docker_backend: host-socket`일 때 Manager는 Runner 컨테이너에 `--group-add <호스트 docker GID>`를 전달합니다(`docker.sock`에서 자동 감지, `runners.docker_gid` / `DOCKER_GID`로 재정의 가능). 이미지에도 `docker` 그룹이 포함되어 있습니다(빌드 인자 `DOCKER_GID`, 기본 999). Runner 이름은 컨테이너 이름으로 정규화되며, 매핑 후 중복 시 충돌합니다.
...
| `runners.container_image` | 컨테이너 모드에서 Runner 이미지(-runner 태그) | `ghcr.io/soulteary/runner-fleet:v1.9.0-runner` |
...
| `RUNNER_IMAGE`, `CONTAINER_IMAGE` | `runners.container_image` | 미설정 시 `MANAGER_IMAGE`에서 유도(`:v1.9.0` → `:v1.9.0-runner`), 그다음 `FLEET_IMAGE_TAG` |
...
| `FLEET_IMAGE_TAG` | 다른 무엇도 정하지 않을 때 기본 Runner 이미지의 tag | `v1.9.0` |
...
이제 두 이미지 모두 PID 1로 `tini`를 실행하므로 이미지 ID가 바뀌고, 컨테이너 모드는 이를 다른
이미지 변경과 똑같이 처리합니다: 중지된 Runner 컨테이너는 다음 시작 때 다시 만들어지고, 실행 중인
것은 "설정 변경됨"으로 표시된 뒤 유휴 상태에서 "컨테이너 재생성"을 누를 때까지 그대로 남습니다.
PID 1을 바꾼 이유는 정지가 Job까지 닿게 하려는 것입니다. Agent가 SIGTERM을 Runner로 전달하고 종료를
기다리며, Job이 남긴 고아 프로세스도 회수되어 좀비로 쌓이지 않습니다. compose 대신 순수
`docker run`으로 Manager를 운영한다면 `--stop-timeout 30`을 추가하세요. Docker의 기본값은 10초로,
Manager가 먼저 Runner를 멈출 여유가 없습니다.

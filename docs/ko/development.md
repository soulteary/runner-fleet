...
`config/tokens/<Runner 이름>`, 가시성 확인이 쓰는 것과 같은 파일 — 입니다. 그래서 등록 해제는
여전히 설치 디렉터리를 지우기 **전에** 수행됩니다. 이전 버전이 그 디렉터리에 남긴 PAT를 그 과정에서
옮겨오기 때문이며, 순서를 뒤집으면 등록 해제할 수단 자체를 조용히 잃게 됩니다.

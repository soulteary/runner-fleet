...
生产环境建议使用具体版本号（如 v1.2.0）；开发可用 `main` tag。
...
| `runners.container_image` | 容器模式下 Runner 镜像（tag 带 -runner） | `ghcr.io/soulteary/runner-fleet:v1.2.0-runner` |
...
**Agent 鉴权**（容器模式）：Manager 会为每个 Runner 在 `<runner 目录>/.agent_token` 写入随机令牌（权限 0600），调用 Agent 时以 `Authorization: Bearer` 带上。Agent 对 `/status`、`/start`、`/stop` 强制校验，同一网络内的其它容器无法再控制 Runner；`/health` 保持开放供容器 HEALTHCHECK 使用。本特性之前创建的容器没有令牌文件，仍按不鉴权运行，重建后生效（`docker rm -f github-runner-<名称>` 后点「启动」）。

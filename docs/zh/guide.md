...
- **界面与它的消息跟随语言，日志固定英文。** 界面以及服务端回给你的那些话——API 消息与提示条——来源依次是 `?lang=`、语言 cookie、`Accept-Language`；不带 `Accept-Language` 的脚本拿到英文。日志固定英文是有意为之，不是没译：一条日志没有请求可依，它的读者是运维，而 grep、Loki 查询与告警规则最怕的就是同一件事换着语言出现。启动自检同样写进日志，行首是 `[preflight …]`。
...
生产环境建议使用具体版本号（如 v1.9.0）；开发可用 `main` tag。
...
| `runners.container_image` | 容器模式下 Runner 镜像（tag 带 -runner） | `ghcr.io/soulteary/runner-fleet:v1.9.0-runner` |
...
| `RUNNER_IMAGE`、`CONTAINER_IMAGE` | `runners.container_image` | 都不设时由 `MANAGER_IMAGE` 推导（`:v1.9.0` → `:v1.9.0-runner`），再退到 `FLEET_IMAGE_TAG` |
...
| `FLEET_IMAGE_TAG` | 其它都没决定时，默认 Runner 镜像的 tag | `v1.9.0` |
...
**注册结果**：写入该 runner 目录 `.registration_result.json`。**GitHub 显示检查**（可选）：把 PAT 放在 `config/tokens/<Runner 名称>`（权限 0600，组织需 `admin:org`、仓库需 `repo`），约每 5 分钟检查，结果写入 `.github_status.json`。旧版本留在 Runner 目录里的 PAT 会被自动搬到那里并从 Runner 目录删除——那个目录会被挂载进该 Runner 的容器，Job 读得到它。 同一次检查还会记录 GitHub 侧该 Runner 是否**正在跑 Job**，列表里显示为「忙碌中」徽标，配置弹窗中单列一行。它与上面同为约 5 分钟一次，因此最多可能滞后 5 分钟；没有 PAT 时保持「未知」，不会说成「空闲」。列表中的「已注册」与「GitHub ✓」均可点击，跳转到该目标在 GitHub 的 Runners 设置页。同一个文件还记录目标仓库是否**公开**：公开的会在列表里挂一个警示色的「公开仓库」徽标，详情对话框多一行「仓库可见性」——公开仓库意味着任何能提 PR 的人都能在这里运行代码。这一项每个仓库每天最多问一次（可见性几乎不变，而匿名 API 每小时只有 60 次），没有 PAT 也能查，有就带上。组织目标不在此列：那边要判断的是 Runner 组允不允许公开仓库，需要 `admin:org`。
...
**传给 Runner 的环境变量**：Manager 与 Agent 启动的每个进程——`run.sh` 及其运行的 Job、`config.sh`、`install-runner.sh`——都不会拿到 `BASIC_AUTH_PASSWORD`、`BASIC_AUTH_USER` 和 `AGENT_TOKEN`，其余变量（包括 `DOCKER_HOST` 与代理设置）原样透传。这能让管理凭据不出现在 Job 日志里，但它不是隔离边界：默认模式下 Job 与 Manager 以同一用户运行（见 [SECURITY.md](../../SECURITY.md)）。

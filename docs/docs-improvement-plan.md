...
**执行状态**：**第 1、2、4 批已完成；第 3 批里不依赖决策的部分已完成。**
只剩两件事：**第 7 节的语言策略决策**（卡着 §2.2、§2.3 与第 3 批的第 11、12 项），
以及 **§1.3 的另一半**（服务端消息补 i18n——那一项动的是真实代码路径，
和文档改动不是一回事，建议单独成批）。
...
**已落地**：三节都进了 `guide.md` 的运维一节（六语言）。写的时候顺着代码核过两处事实，
都不是凭常识能猜对的：升级时**版本 tag 会自己被 `docker create` 拉下来，而 `:main` 或
重新构建过的同名 tag 不会**（本地已经解析得到，会直接复用旧镜像，必须自己先 `docker pull`）；
以及 `runners.resources` 是唯一一个不重建就能作用到已有容器的配置项（启动时走 `docker update`）。
备份一节则纠正了 README 那句「配置就是你的备份」——对配置成立，对**身份**不成立：
Runner 的凭据在它自己的安装目录里。
...
### 3.3 CHANGELOG 的条目长到没法扫  ◐ 约定已定，历史条目按原计划不动
...
**已落地的那一半**：新条目的写法约定写进了 `CONTRIBUTING.md`——一句话说清改了什么、
对升级的人意味着什么，外加一个链接；推理留在代码注释与提交信息里，那本来就是这个仓库
放它们的地方。**没有落地的那一半**：`docs/decisions/` 没有建。本节原本的设想是把设计说明
迁过去，但历史条目按同一节的结论不动，于是那个目录会是空的——建一个没有内容的约定，
比不建更糟。等真有一条说明长到该独立成文时再建它。
...
**已落地**：`development.md` 新增「架构」一节（六语言），含一张 mermaid 关系图。
图里的标签在所有译文里保持英文——那些是进程名、路径与端点，翻译标识符只会更难 grep。
正文讲三件事：Manager 只编排不承载 Runner；状态因为跨 PID namespace 所以走 HTTP，
探不到时置 `unknown` 而不是 `installed`；以及**九个字段在 `docker create` 那一刻定死**
（容器名、镜像、网络、挂载目录、Job Docker 后端、DinD 主机、docker GID、Agent 令牌、资源上限），
这正是漂移检测存在的全部理由。
...
### 第 4 批：长期资产  ✅ 已完成（第 19 项见 §3.3 的说明）

...
- **画面とそのメッセージは言語に追従し、ログは英語に固定です。** UI と、サーバーが返すもの——API メッセージとトースト——は `?lang=`、言語 cookie、`Accept-Language` の順で決まり、`Accept-Language` を送らないスクリプトには英語が返ります。ログが英語なのは訳し残しではなく決めごとです。ログ行には追従すべきリクエストがなく、読者は運用担当であり、grep・Loki のクエリ・アラートルールは同じ事象が実行ごとに違う言語で現れると壊れます。起動時セルフチェックもログに出力され、行頭に `[preflight …]` が付きます。
...
本番では特定バージョン（例: v1.9.0）を使用してください。開発時は `main` タグが使えます。
...
Runner イメージ: Manager と同じ名前で `-runner` タグ（本番はバージョン例 v1.9.0-runner、開発は main-runner）、またはローカルビルド: `docker build -f Dockerfile.runner -t ghcr.io/soulteary/runner-fleet:v1.9.0-runner .`。Manager はホストの Docker（`docker.sock` のマウント）を使う必要があり、`DOCKER_HOST` で DinD にはしないでください。Compose ではホストの docker GID 用に `group_add` または `user: "0:0"` を使用。`job_docker_backend: host-socket` の場合、Manager は Runner コンテナに `--group-add <ホストの docker GID>` を渡します（`docker.sock` から自動検出、`runners.docker_gid` / `DOCKER_GID` で上書き可）。イメージ側にも `docker` グループを用意しています（ビルド引数 `DOCKER_GID`、既定 999）。Runner 名はコンテナ名に正規化され、マッピング後の重複は衝突します。
...
| `runners.container_image` | コンテナモード時の Runner イメージ（-runner タグ） | `ghcr.io/soulteary/runner-fleet:v1.9.0-runner` |
...
| `RUNNER_IMAGE`、`CONTAINER_IMAGE` | `runners.container_image` | 未設定なら `MANAGER_IMAGE` から導出（`:v1.9.0` → `:v1.9.0-runner`）、次に `FLEET_IMAGE_TAG` |
...
| `FLEET_IMAGE_TAG` | ほかに決め手がないときの既定 Runner イメージの tag | `v1.9.0` |
...
**登録結果**: その Runner ディレクトリの `.registration_result.json` に書き込み。**GitHub 表示チェック**（任意）: PAT を `config/tokens/<Runner 名>`（モード 0600。組織は `admin:org`、リポジトリは `repo` が必要）に置くと約 5 分ごとにチェックし、結果は `.github_status.json` に書き込み。旧バージョンで Runner ディレクトリに残った PAT は自動でそこへ移され、Runner ディレクトリからは削除されます——そのディレクトリは Runner のコンテナにマウントされ、ジョブから読めてしまうためです。 同じチェックで GitHub 側の Runner が**ジョブ実行中**かどうかも記録し、一覧では「ジョブ実行中」バッジ、設定ダイアログでは独立した行として表示します。同じ約 5 分間隔のため最大 5 分遅れることがあり、PAT がない場合は「不明」のままで、アイドルとは報告しません。一覧の「登録済み」と「GitHub ✓」はいずれも対象の Runners 設定ページへのリンクです。同じファイルには対象リポジトリが**パブリック**かどうかも記録されます。パブリックなら一覧に警告色の「パブリックリポジトリ」バッジが付き、ダイアログに「リポジトリの可視性」の行が増えます——パブリックリポジトリでは、プルリクエストを開ける人なら誰でもここでコードを実行できるからです。この確認はリポジトリごとに 1 日 1 回までです（可視性はほとんど変わらず、匿名 API は 1 時間に 60 回しか使えません）。PAT がなくても動き、あれば使います。組織ターゲットは対象外です。そちらで判断すべきなのは Runner グループがパブリックリポジトリを許可しているかどうかで、`admin:org` が必要になります。
...
両方のイメージが PID 1 として `tini` を実行するようになったため、イメージ ID が変わります。
コンテナモードはこれを通常のイメージ変更と同じように扱います: 停止中の Runner コンテナは次回起動時に
作り直され、実行中のものは「設定が変更されました」と印が付き、アイドル時に「コンテナを再作成」を
押すまでそのままです。PID 1 を差し替えたのは、停止が Job まで届くようにするためです。Agent が
SIGTERM を Runner に転送して終了を待ち、Job が残した孤児プロセスも回収されるので、ゾンビとして
溜まり続けることがなくなります。compose ではなく素の `docker run` で Manager を動かしている場合は
`--stop-timeout 30` を足してください。Docker 自身の既定値は 10 秒で、Manager が先に Runner を
停止する余裕がありません。
...
重要なのはファイルの権限だけではなく、ディレクトリの権限も同じです。ディレクトリが UID 1001 から
書き込み可能であれば、Manager は `config.yaml` を原子的に置き換えるので、保存中の読み取りが
書きかけの内容を読むことも、途中のクラッシュで切り詰められることもありません。ファイルだけが
書き込み可能な場合、またはそのファイルだけを bind mount している場合
（`-v ./config.yaml:/app/config/config.yaml`）は、その場での書き込みに戻り、警告を一度だけ出します。

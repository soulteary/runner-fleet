...
- **画面とそのメッセージは翻訳済み、ログは未翻訳。** UI と、サーバーが返すもの——API メッセージとトースト——は言語に追従します。判定は `?lang=`、言語 cookie、`Accept-Language` の順で、`Accept-Language` を送らないスクリプトには英語が返ります。**ログ行は依然として中国語**で、起動時セルフチェックも同様です。行頭に `[preflight …]` が付くので中国語が読めなくても拾えます。より深い層で生成される一部のメッセージ（GitHub の登録解除結果など）には中国語が残ります。これは既知の制約であり、訳し残しではありません。

...
`config/tokens/<Runner 名>`、可視性チェックが使うのと同じファイルです。そのため登録解除は
今も インストールディレクトリを削除する**前**に実行されます。旧バージョンがそこに残した PAT は
その際に移動されるからで、順序を誤ると登録解除する手段そのものを黙って失います。
...
このファイルと同じ場所に、メンテナ向けの作業記録が 2 つあります。どちらも製品の挙動ではなく
ビルド上の判断を記録したものなので、意図的に訳していません:
[CI シェルの ci-recipes への移行](../ci-recipes-migration.md) はその移行の監査（どのシェルが
移すべきものだったか、移植で直った 4 つの欠陥、その代償）、
[ドキュメント改善計画](../docs-improvement-plan.md) はここに挙げた各検査の出どころで、
何が済んでいて何が残っているかが書いてあります。
